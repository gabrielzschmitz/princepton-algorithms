/*
  Program Description:
  --------------------
  BoggleGame.java implements a graphical user interface (GUI) for playing
  Boggle against a computer opponent of various difficulties. The game can
  be launched from the command line, where the default size of the board
  for that game must be specified.

  Features:
  ---------
  - GUI interface for playing Boggle against a computer opponent
  - Supports launching from the command line with customizable board size
  - Provides various difficulty levels for the computer opponent
  - Allows users to interact with the game using mouse and keyboard inputs
  - Dynamically adjusts the board size based on user input
  - Provides error handling and usage instructions for command-line launch

  Usage Instructions:
  -------------------
  1. Compile the program using the command: javac-algs4 BoggleGame.java
  2. Launch the game using one of the following methods:
     - java-algs4 BoggleGame [m n]: Specify the size of the board as m rows and n
	 columns
     - java-algs4 BoggleGame: Launch with default 4x4 board size
  3. Play the game using the graphical interface provided
  4. Use mouse and keyboard inputs to interact with the game

  Dependencies:
  -------------
  - The BoggleGame class depends on the BoggleSolver.java and BoggleBoard.java
    classes for solving Boggle boards and managing board data.
  ...
  Note:
  -----
  - Expect some compiler warnings with Java 7 due to javax.swing.JList being
    a parameterized type in Java 7 but not in Java 6.
*/
